...
export default function Home() {
  ...
  const isFieldFilled = (): string => {
    if (isAll) {
      ...
      return isSomeDataInputed ? '' : 'btn-disabled bg-[#50514F] text-gray-400';
    } else {
      return allValue.length > 0 ? '' : 'btn-disabled bg-[#50514F] text-gray-400';
    }
  };
  ...
  const reset = () => {
    ...
    //   return item;
    // });
    // console.log("🚀 ~ file: index.tsx ~ line 147 ~ initialData ~ initialData", initialData)
    // setData(initialData);
    setAllValue(0);
    setNote('');
  }
  ...
  const updateUserMoney = async () => {
    ...
    if (!isAll && allValue) {
      ...
      const newData = [...data] as any;
      // Filter user with money > 0
      const updatedUser = newData.filter((item: any) => item.money > 0);
          ...
          <div className="flex flex-row mt-6 w-full min-[418px]:justify-center">
            <div className="w-full flex flex-col">
              ...
              <div className={`${isAll ? 'block min-[530px]:mx-auto': 'hidden'}`}>
                {paidUsers.length && paidUsers.map((user: any, index: number) => (
                  <div className="flex flex-row">
                    <InputUnit id={index} name={user.name} value={data.money} placeholder={`đã thiệt hại ...`} onChange={(e) => {onValueChanged(e, index)}}/>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4 min-[530px]:mx-auto">
                ...
                <button onClick={(e) => {updateUserMoney()}} className={`btn bg-[#6290C8] min-[530px]:min-w-[340px] w-full border-[#376996] text-white ${isFieldFilled()} ${loading? 'loading btn-disabled': ''}`}>{loading ? 'đợi xíu đang lụm': 'Lụm'}</button>
